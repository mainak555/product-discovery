...
45. **OpenTelemetry is the project-wide tracing standard**: all I/O layers emit spans into a single trace per request via `@traced_function` / `traced_block` from `core/tracing.py`, plus three category-gated auto-instrumentation toggles — `OTEL_INSTRUMENT_HTTP` (Django + `requests`, default **on**), `OTEL_INSTRUMENT_MONGO` (pymongo, default **off** — high-volume DB spans), `OTEL_INSTRUMENT_AGENTS` (AutoGen event-log → span bridge, default **on**). The OTLP exporter is **pluggable** — Langfuse is wired today via `_build_langfuse_exporter()`; swapping backends is a one-function change in `_build_exporter()`. Span payloads are always set through `set_payload_attribute()` so redaction (`redact_payload()`) and truncation (`OTEL_MAX_PAYLOAD_BYTES`, default 32 KB) run uniformly. Console span output is opt-in via `OTEL_CONSOLE_EXPORTER` (`off` / `error` / `all`); `LOG_LEVEL=DEBUG` upgrades the default to `all`. Active toggles are echoed on the `tracing.enabled` startup log line (`instrument_http`, `instrument_pymongo`, `instrument_agents`, `console_span_mode`). Treat the OTLP backend as PII-bearing (Trello card descriptions, Jira issue summaries are forwarded as payloads). Full contract, env vars, and validation checklist live in [docs/observability.md](docs/observability.md).
...
50. **MCP tool scope is per-agent + project-level shared**: each assistant agent stores `mcp_tools` ∈ {`none`, `shared`, `dedicated`}. `dedicated` requires a non-empty `mcp_configuration` (`{"mcpServers": {...}}`) on the same agent. `shared` requires a non-empty project-level `shared_mcp_tools` (`{"mcpServers": {...}}`). Validation must reject `shared` agents when `shared_mcp_tools` is empty. SSE transport is **not supported** (deprecated upstream); only stdio (`{command,args,env}`) and streamable HTTP (`{transport:"http",url,headers}`) are accepted. See [docs/mcp_integration.md](docs/mcp_integration.md) and [`.agents/skills/mcp_tool_integration/SKILL.md`](.agents/skills/mcp_tool_integration/SKILL.md).
51. **MCP runtime wiring lives in `agents/mcp_tools.py`**: `agents/team_builder.py` calls `resolve_mcp_servers_for_agent()` + `build_mcp_workbenches()` and passes the resulting workbenches to `AssistantAgent(workbench=...)`. Lifecycle is owned by `agents/runtime.py`: `evict_team()` must call `close_session_workbenches(session_id)`. Never construct `McpWorkbench` from `server/`.
52. **MCP secrets must never be logged or traced**: `args`, `env`, `headers`, and `url` query strings of MCP server entries are credentials by policy. Logs and OTel span attributes may include only `scope`, `server_count`, `server_names`, and a stable `fingerprint` hash. Use `set_payload_attribute()` + `redact_payload()` for any payload that must be carried.
53. **MCP deployment topology is governed by `deployments/`**: `standalone/` bundles Node in the app image (single-container hosts); `compose/` and `k8s/` use a Python-only app container plus a Node-based **mcp-gateway** sidecar exposing each MCP server over streamable HTTP. New deployment targets must follow the same split and document MCP transport reachability.
54. **MCP secrets-by-reference is mandatory for all credential material**: never embed raw API keys, tokens, or passwords directly in `shared_mcp_tools` or per-agent `mcp_configuration`. Store them under the project-level `mcp_secrets` dict (`{KEY: value}`) where `KEY` matches `^[A-Z][A-Z0-9_]*$` (UPPER_SNAKE), and reference them from any string value in `mcpServers` entries (including `args`, `env` values, `url`, `headers` values) using the `{KEY_NAME}` placeholder. `validate_project()` rejects any placeholder that has no matching `mcp_secrets` key. Substitution happens **only** at runtime in `agents/mcp_tools.py::_substitute_secrets()` immediately before `McpWorkbench` construction; `server/` code never sees substituted values.
55. **MCP secrets follow the SECRET_MASK contract**: `normalize_project()` returns `{KEY: SECRET_MASK}` for every stored secret so the form re-renders password inputs without leaking values; `_restore_masked_secrets()` swaps `SECRET_MASK` back to the DB value on save (keys absent from the submitted payload are treated as user deletions and dropped). Readonly view (`config_readonly.html`) MUST NOT render secret values — only an optional `🔒 N secrets configured` count badge listing key names.
56. **MCP secret fingerprint is computed over placeholders, not values**: `build_mcp_workbenches()` computes `_server_fingerprint()` over the placeholder `mcpServers` dict (pre-substitution), so the OTel `fingerprint` attribute stays stable across secret rotations and never carries credential material. Logs and span attributes for MCP must continue to expose only `scope`, `server_count`, `server_names`, `fingerprint`.
57. **Key/value form pattern is the only blessed UI for repeating KEY → value rows**: MCP Secrets, Trello Custom Fields, and any future provider list-of-pairs control (Jira custom fields, n8n inputs, header maps) must follow [`.agents/skills/key_value_form_pattern/SKILL.md`](.agents/skills/key_value_form_pattern/SKILL.md) — `1fr 2fr auto` grid, `+ Add` button in the subsection header, `chat-session-item__delete` × button per row, JS reindex on remove, and (for sensitive values) `<input type="password">` with the SECRET_MASK round-trip.
58. **Datetime storage standard is mandatory**: all datetime fields in MongoDB must be stored as BSON Date objects (`datetime.now(timezone.utc)`) — never as ISO strings or bare `HH:MM` strings. Use `_utc_now()` from `server/services.py` to produce values. Read paths must coerce BSON datetimes to ISO strings via `_coerce_dt_to_iso()`. Templates must wrap values in `<time class="local-time" data-utc="ISO">` elements; `window.renderLocalTimes()` in `app.js` converts to browser-local display. JS live injections use `new Date().toISOString()` + `window.renderLocalTimes()`. Export pipeline sub-document fields (`discussions[].exports.*.updated_at`, `pushed_at`) are exempt until a dedicated migration. See [`.agents/skills/datetime_storage/SKILL.md`](.agents/skills/datetime_storage/SKILL.md) for the full contract and new-field checklist.
59. **Active run coordination is Redis-backed and fail-fast**: chat run ownership must be coordinated by `agents/session_coordination.py` using one lease per `session_id` plus heartbeat and cross-instance cancel signaling. `POST /chat/sessions/<id>/run/` must fail fast when Redis is unavailable and must not enter `running`. MongoDB remains the durable source for `chat_sessions.agent_state` and discussion history used for resume.
60. **Run lifecycle refactors require the active-session skill**: before changing chat run/start/stop/lease/cancel behavior, follow [`.agents/skills/active_session_coordination/SKILL.md`](.agents/skills/active_session_coordination/SKILL.md).